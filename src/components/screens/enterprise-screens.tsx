...
export const ExecutiveDemoScreen = () => (
  <div className="space-y-8">
    ...
    <Card className="border-none bg-[var(--vidda-accent)] shadow-none"><CardContent className="flex flex-col gap-5 p-7 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-2xl font-extrabold text-white">Explore a pilot for your compliance team</p><p className="mt-1 text-sm text-white/75">Start with one policy change, one role group and measurable readiness goals.</p></div><Button size="lg" className="gap-2 bg-white text-[var(--vidda-primary)] hover:bg-white/90" onClick={() => toast.success("Pilot brief created for NordBank International")}>Create pilot brief<Icon icon="solar:arrow-right-linear" /></Button></CardContent></Card>
  </div>
);
