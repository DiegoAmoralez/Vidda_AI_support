  ...
  return (
    <Section title="My capability profile" eyebrow="Updated today" description="Scores show your ability to apply controls in practical situations, not course completion.">
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        <Card className="shadow-none"><CardContent className="p-6"><div className="flex items-start justify-between"><div><p className="text-sm font-extrabold">Readiness trend</p><p className="text-xs text-muted-foreground">Six-month capability signal</p></div><p className="metric-number text-3xl font-extrabold">{score}%</p></div><div className="mt-5 h-72"><ResponsiveContainer width="100%" height="100%"><LineChart data={employeeTrend}><CartesianGrid vertical={false} stroke="#d9e2ef" /><XAxis dataKey="month" axisLine={false} tickLine={false} fontSize={11} /><YAxis domain={[50, 100]} axisLine={false} tickLine={false} fontSize={11} /><Tooltip /><Line type="monotone" dataKey="score" stroke="#2c4269" strokeWidth={3} dot={{ fill: "#0087c1", stroke: "#2c4269", strokeWidth: 2 }} /></LineChart></ResponsiveContainer></div></CardContent></Card>
        <Card className="shadow-none"><CardContent className="p-6"><p className="text-sm font-extrabold">Capability balance</p><p className="text-xs text-muted-foreground">Current practical scores</p><div className="mt-4 h-72"><ResponsiveContainer width="100%" height="100%"><RadarChart data={radarData}><PolarGrid /><PolarAngleAxis dataKey="capability" fontSize={10} /><Radar dataKey="score" stroke="#2c4269" fill="#0087c1" fillOpacity={0.35} /></RadarChart></ResponsiveContainer></div></CardContent></Card>
      </div>
